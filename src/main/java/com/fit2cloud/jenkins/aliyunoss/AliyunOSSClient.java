...
public class AliyunOSSClient {
			...
			while (strTokens.hasMoreElements()) {
				...
				if (paths.length != 0) {
					for (FilePath src : paths) {
						...
						listener.getLogger().println(
								"Aliyun OSS Uploaded Object Link:" + baseUploadUrl + "/" + key);
						filesUploaded++;
					}
				}
			}
	...
	private static final String[] COMMON_CONTENT_TYPES = { ".js",
			"application/js", ".json", "application/json", ".svg", "image/svg+xml",
			".woff", "application/x-font-woff", ".woff2", "application/x-font-woff",
			".ttf", "application/x-font-ttf" };

	// http://www.rgagnon.com/javadetails/java-0487.html
	// we don't use the more robust solutions (checking magic headers) here,
	// because the file stream might be
	// loaded remotely, so that would be time consuming in checking, even hangs
	// the Jenkins build in my test.
